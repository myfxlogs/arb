...
以下项目经架构评审确认豁免，未来审计无需再次提出：

| # | 项目 | 豁免原因 |
|---|------|---------|
| 11 | `broker_accounts.password` 明文存储 | 运行时必须将原始密码透传给 mtapi.io 进行经纪商认证；任何加密（除启动时解密）都不会增加安全边界，因为 key 必须在同一进程内。当前设计为：存储端按项目惯例管理，运行时内存中短暂持有。 |
